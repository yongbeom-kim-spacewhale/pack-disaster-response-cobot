...
- 기존 497장·3,557개 BBox와 초기 데이터로 Roboflow 자동 마스크 모델 구성
- 추가 이미지에 자동 마스크를 생성한 뒤 수동 검수하고, 취약 클래스와 빈 배경을 보강해 Dataset v2 633장·3,623 instances 구성
...
33/36은 장면 단위 검출 여부이며 프레임 단위 정확도가 아닙니다. 처리시간도 AI worker 내부 측정값이며 전체 시스템 지연을 뜻하지 않습니다.
